...
The coordinator now also exposes a first-pass project-open flow at `/api/projects/{projectId}/open/{machineId}`. That brokered path asks the selected runner to resolve or bootstrap the project workspace under its workspace root, clones the configured repository when needed, updates the project's workspace mapping, and opens a project-rooted terminal session so the companion can attach to a real machine/project combination instead of a generic ad-hoc shell. When the repository is hosted on GitHub and the runner is already authenticated with `gh auth login`, the bootstrap flow now prefers `gh repo clone` so private GitHub repositories can be opened without separate manual git credential setup on the runner.
