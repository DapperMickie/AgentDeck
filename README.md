...
The coordinator now also brokers those orchestration, viewer-session, and virtual-device APIs back out under `/api/machines/{machineId}/...`, and the companion project page now uses that control-plane path to open a project on a runner, choose a ready host for each launch profile, choose emulator/simulator targets when required, queue real run/debug jobs, inspect live job state, and list viewer-session entry points without talking to runner URLs directly. That page now also surfaces compact control-plane warnings for candidate machines, stops treating protocol-incompatible runners as valid project/open debug targets, and gates launch-profile machine readiness on the remote viewer surfaces each workflow actually needs (for example Window, Emulator, Simulator, or VS Code viewers). The linked project-session page also carries the host machine's control-plane warnings forward so active sessions still show protocol/update/workflow health context. The companion now also has a dedicated in-app remote-viewer page for managed relay sessions, and those live frames/input now terminate at coordinator-hosted hubs instead of exposing runner `ConnectionUri` or viewer tokens to the client.
